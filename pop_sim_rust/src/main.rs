    ...
    pub fn update_sim(&mut self) {
        for (i, person) in self.people.iter_mut().enumerate() {
            if person.age != -1 {
                ...
                if person.love_vec[0] == -1 && person.age > 12 * 12 {
                    ...
                    if lover != i && self.people[lover].love_vec[0] == -1 && person.gender != self.people[lover].gender {
                        person.love_vec[0] = lover as i64;
                        self.people[lover].love_vec[0] = i as i64;
                    }
                }

                if person.love_vec[1] != -1 {
...
//
//     pub fn update_sim(&mut self) {
//         let people_iter = self.people.iter();
//         for id in 0..self.people.len() {
