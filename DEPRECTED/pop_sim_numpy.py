...
class Sim:
    people = np.array([], dtype='int32')
    dead_people = np.array([])
    names = np.array([])
    ...
    def createPerson(self, gender_choice=-1, id_who_created=0):
        ...
        np.append(both_gender_names[gender], self.names)

        # Gives person someone they will love, if they are in a relationship and affection
        # love_lst = np.array([-1, -1, 100])  # 100 is a test !!!

        if id_who_created == 0:
            self.temp_person = np.array([population, 0,
                                         gender, -1, -1, 100,
                                         id_who_created, id_who_created, 0], dtype='int32')
        else:
            self.temp_person = np.array([population,  0,
                                         gender, -1, -1, 100, id_who_created - 1,
                                         self.people[id_who_created - 1][4], 0], dtype='int32')
        ...
        np.append(self.temp_person, self.people)
        print(self.temp_person)
    ...
    def printPeople(self):
        ...
        for self.p in self.people:
            ...
            print(f"Name: {self.names[self.p[0]]}\n"
    ...
    def updateSim(self, amount_of_time):
        ...
        for self.time in track(range(amount_of_time), "Total load\n"):
            ...
            np.append(pop, self.people.size)
            tp += 1
            np.append(mp, tp)
...
while True:
    ...
    if time_amount == "g":
        ...
        if print_every_person:
            ...
            print(f"Population: {Sim.people.size}\n")
        # c.print()
